...
function processDir(dir: string) {
  ...
  for (const file of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      // Specifically fix input tags to have correct text colors
      // The issue is: inputs don't have text color set, so they inherit, which sometimes causes white-on-white.
      // But now that backgrounds are bg-white/5, white text will show up well.
      // Let's add text-nexus-text explicitly to all simple inputs:
