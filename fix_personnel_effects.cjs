...
// Replace useDI with useDependencies
content = content.replace(/import \{ useDI \} from '\.\.\/core\/di\/DependencyProvider';/g, "import { useDependencies } from '../core/di/DependencyProvider';");
content = content.replace(/useDI\(\)\.facades/g, "useDependencies().facades");

// Replace the useEffect block for fetchData and supabase channel
const oldEffectRegex = /useEffect\(\(\) => \{\s*if \(!currentCompany\) return;\s*const fetchData = async \(\) => \{[\s\S]*?\}\s*\}, \[currentCompany\]\);/;
...
console.log('Fixed useEffect and useDependencies.');
