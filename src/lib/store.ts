    ...
    (set, get) => ({
      ...
      refreshAffiliations: async () => {
        ...
        while (attempts < maxAttempts) {
          try {
            if (!sb) {
              console.warn("Nexus Recovery: Supabase unavailable, falling back to Firestore.");
              let mappedCompanies: Company[] = [];
              const q1 = query(collection(db, 'companies'), where('ownerId', '==', firebaseUser.uid));
              
              const [snap1, snap2] = await Promise.all([
                getDocs(q1),
                firebaseUser.email ? getDocs(query(collection(db, 'companies'), where('memberEmails', 'array-contains', firebaseUser.email))) : Promise.resolve({ docs: [] })
              ]);
              ...
              mappedCompanies = uniqueDocs.map(d => {
                const data = d.data();
                const isOwner = data.ownerId === firebaseUser.uid;
                return {
                  id: d.id,
                  name: data.name,
                  ownerId: data.ownerId,
                  ownerEmail: data.ownerEmail,
                  role: isOwner ? 'OWNER' : 'Collaborateur',
                  permissions: ['*'],
                  status: 'active',
              ...
              set({ 
                ...
                userProfile: { id: firebaseUser.uid, email: firebaseUser.email, fullname: firebaseUser.displayName },
                initialized: true 
              });
