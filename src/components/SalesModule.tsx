...
export default function SalesModule() {
  ...
  useEffect(() => {
    ...
    return () => { unsubSales(); unsubInvoices(); };
  }, [currentCompany]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-4 border border-slate-200 rounded-xl shadow-sm">
           ...
           <p className="text-2xl font-black text-slate-900">{calculateTotalRevenue().toLocaleString()} €</p>
        </div>
        <div className="bg-white p-4 border border-slate-200 rounded-xl shadow-sm">
           ...
           <p className="text-2xl font-black text-slate-900">{calculatePendingRevenue().toLocaleString()} €</p>
        </div>
      </div>
          ...
          <Table headers={['Date', 'Type', 'Article/Service', 'Qté', 'Prix U.', 'Total', 'Statut', 'Actions']}>
            {sales.map(sale => (
              <TableRow key={sale.id}>
                ...
                <span className="text-slate-600 font-mono">{sale.price.toLocaleString()} €</span>
                <span className="font-black text-slate-900 font-mono">{sale.total.toLocaleString()} €</span>
          ...
          <Table headers={['N° Facture', 'Vente Liée', 'Date', 'Montant', 'Statut']}>
            {invoices.map(inv => (
              <TableRow key={inv.id}>
                ...
                <span className="font-black text-slate-900 font-mono">{inv.amount.toLocaleString()} €</span>
          ...
          <div className="bg-white rounded-2xl p-8 max-w-lg w-full shadow-2xl border border-slate-100">
            ...
            <form onSubmit={handleCreateSale} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                 ...
                 <div className="space-y-1.5">
                   <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Prix Unitaire (€)</label>
              ...
              <div className="pt-4 border-t border-slate-100 flex justify-between items-center text-slate-800">
                 ...
                 <span className="text-xl font-black">{(Number(formData.quantity || 0) * Number(formData.price || 0)).toLocaleString()} €</span>
              </div>
