...
export default function SalesModule({ user }: { user: any }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden bg-nexus-surface rounded-[2rem] p-8 sm:p-12 text-nexus-text shadow-2xl border border-nexus-border/30 group">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
          <div className="max-w-xl">
            ...
            >
              ...
              <p className="text-nexus-text-muted text-sm font-medium leading-relaxed max-w-2xl">
                Supervisez vos cycles de vente, gérez les factures et analysez vos performances financières avec précision.
              </p>
            </motion.div>
          </div>
          <div className="flex bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5 backdrop-blur-md p-1.5 rounded-[1.5rem] border border-nexus-border/50 shrink-0 gap-1 overflow-x-auto scrollbar-hide max-w-full">
             {availableTabs.map(item => (
               <button 
                 ...
                 className={cn(
                   ...
                   activeTab === item.id ? "bg-nexus-accent text-white shadow-[0_10px_20px_rgba(91,140,255,0.2)]" : "text-nexus-text-muted hover:text-nexus-text hover:bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5"
                 )}
               >
                 {item.label}
               </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <motion.div 
          ...
          className="bg-nexus-surface p-6 border border-nexus-border/30 rounded-[2rem] shadow-2xl group hover:border-nexus-success/30 transition-all"
        >
        ...
        <motion.div 
          ...
          className="bg-nexus-surface p-6 border border-nexus-border/30 rounded-[2rem] shadow-2xl group hover:border-nexus-warning/30 transition-all"
        >
           <div className="flex justify-between items-center mb-4">
      ...
      <div className="bg-nexus-surface border border-nexus-border/30 rounded-[2.5rem] shadow-2xl overflow-hidden">
        <div className="p-6 border-b border-nexus-border/30 flex flex-col sm:flex-row justify-between items-center bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5 backdrop-blur-sm gap-4">
          <div className="flex items-center gap-3 w-full sm:w-auto">
             <div className="bg-nexus-bg border border-nexus-border/50 rounded-2xl px-5 py-3 flex items-center gap-3 w-full sm:w-64 focus-within:border-nexus-accent transition-colors group">
        ...
        {activeTab === 'orders' && (
          <div className="p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5">
            {openOrders.map(order => (
              <motion.div 
                key={order.id}
                initial={{ opacity: 0, scale: 0.95 }}
                animate={{ opacity: 1, scale: 1 }}
                className="bg-nexus-surface p-7 rounded-[2.5rem] border border-nexus-border/30 shadow-2xl hover:border-nexus-accent/30 transition-all group relative overflow-hidden"
              >
                  ...
                  {order.items?.map((item: any, i: number) => (
                    <div key={i} className="flex justify-between items-center text-[11px] group/item">
                      <span className="text-nexus-text-muted font-bold flex items-center gap-2">
                        <span className="w-5 h-5 bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5 rounded-lg flex items-center justify-center text-[9px] font-black text-nexus-accent">{item.quantity}</span>
                        {item.name}
                      </span>
                ...
                <div className="relative z-10 pt-6 border-t border-nexus-border/30 flex justify-between items-center mb-6">
            ...
            <button 
              ...
              className="bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5 border-2 border-dashed border-nexus-border/50 rounded-[2.5rem] p-10 flex flex-col items-center justify-center gap-4 text-nexus-text-muted hover:text-nexus-accent hover:border-nexus-accent/50 transition-all group"
            >
              <div className="p-4 bg-nexus-surface rounded-3xl shadow-2xl group-hover:scale-110 transition-transform border border-nexus-border/30"><Plus size={32} /></div>
            ...
            <Table headers={['Date', 'Type', 'Désignation', 'Qté', 'Prix U.', 'Total', 'Statut', 'Actions']}>
              {sales.map(sale => (
                <TableRow key={sale.id}>
                  ...
                  <span className={cn("px-2.5 py-1 rounded-xl text-[8px] uppercase font-black border", sale.status === 'completed' ? "bg-nexus-success/10 text-nexus-success border-nexus-success/20" : "bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5 text-nexus-text-muted border-nexus-border/50")}>
        ...
        {activeTab === 'pos' && (
          <div className="p-6 grid grid-cols-1 lg:grid-cols-4 gap-6 bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5 border-t border-nexus-border/50">
            {/* Commandes en Cours Sidebar */}
            <div className="lg:col-span-1 bg-nexus-surface rounded-2xl border border-nexus-border/50 border border-nexus-border/50 shadow-sm p-4 flex flex-col h-[500px]">
               <div className="flex justify-between items-center mb-4">
               ...
               <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                 <button 
                   ...
                   className={cn(
                     ...
                     activeOrderId === null ? "bg-nexus-accent border-slate-900 text-white" : "bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5 border-nexus-border/50 hover:border-white/20 text-nexus-text"
                   )}
                 >
                   <div>
                     <span className="text-xs font-bold block">Vente Rapide</span>
                     <span className={cn("text-[10px]", activeOrderId === null ? "text-nexus-text-muted" : "text-nexus-text-muted/80")}>Client de passage</span>
                   </div>
                 </button>
                 ...
                 {openOrders.map(order => (
                   <button 
                     ...
                     className={cn(
                       ...
                       activeOrderId === order.id ? "bg-nexus-accent text-white hover:bg-nexus-accent-hover shadow-lg hover:shadow-nexus-accent/20 border-blue-600 text-white" : "bg-nexus-surface border-nexus-border/50 hover:border-blue-300 text-nexus-text"
                     )}
                   >
                     <div>
                     ...
                     <div 
                       ...
                       className={cn(
                         ...
                         activeOrderId === order.id ? "text-white hover:bg-slate-500/20 dark:bg-slate-500/20 dark:bg-white/20" : "text-red-500 hover:bg-red-50"
                       )}
                     >
                       <Trash2 size={14} />
                     </div>
               ...
               <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 gap-3">
                 {resources.filter(r => r.type === 'Stock').map(r => (
                   <button 
                     ...
                     className="bg-nexus-surface p-3 rounded-xl border border-nexus-border/50 hover:border-blue-400 hover:shadow-md transition-all text-left flex flex-col justify-between h-24 relative overflow-hidden group"
                   >
                     ...
                     <div className="absolute inset-0 bg-nexus-accent text-white hover:bg-nexus-accent-hover shadow-lg hover:shadow-nexus-accent/20/5 opacity-0 group-hover:opacity-100 transition-opacity" />
                   </button>
                 ))}
            ...
            {/* Cart */}
            <div className="bg-nexus-surface rounded-2xl border border-nexus-border/50 border border-nexus-border/50 shadow-sm p-4 flex flex-col h-[500px]">
               ...
               <div className="flex-1 overflow-y-auto space-y-2 pr-2">
                 {currentCart.length === 0 ? (
                   ...
                   currentCart.map((item: any) => (
                     <div key={item.id} className="flex justify-between items-center bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5 p-2 rounded-lg border border-nexus-border/30">
                       <div className="flex-1 min-w-0 pr-2">
                         ...
                         <div className="flex items-center gap-2 mt-1">
                           ...
                           }} className="w-5 h-5 bg-nexus-surface border border-nexus-border rounded flex items-center justify-center font-bold text-xs hover:bg-slate-100">-</button>
                       ...
                       <div className="text-right shrink-0">
                         <div className="text-xs font-black text-nexus-text border-b border-transparent group-hover:border-nexus-border/50 focus-within:border-blue-400 pb-0.5">
              ...
              {payments.map(pay => (
                <TableRow key={pay.id}>
                  ...
                  <button 
                    ...
                    className="font-black text-nexus-text hover:text-nexus-accent hover:underline text-left italic border-b border-nexus-border/50"
                  >
                    {invoices.find(i => i.id === pay.invoiceId)?.invoiceNumber || 'FLUX_EXT'}
                  </button>
                  <span className="px-2.5 py-1 bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5 text-nexus-text-muted rounded-xl text-[9px] uppercase font-black tracking-widest border border-nexus-border/50">{pay.method}</span>
                  <span className="text-[10px] font-black text-nexus-text tracking-tighter">
        ...
        {activeTab === 'catalog' && (
          <div className="p-8 space-y-8 bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5">
            <div className="flex gap-8 border-b border-nexus-border/30">
              <button 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              {(catalogType === 'product' ? resources : services).map((item: any) => (
                <motion.div 
                  ...
                  initial={{ opacity: 0, scale: 0.95 }}
                  animate={{ opacity: 1, scale: 1 }}
                  className="bg-nexus-surface p-7 rounded-[2.5rem] border border-nexus-border/30 shadow-2xl group relative overflow-hidden hover:border-nexus-accent/30 transition-all"
                >
                  {userRole === 'admin' && (
                    <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-all flex gap-2 z-20">
                      <button onClick={() => { setCatalogFormData(item); setIsAddingCatalogItem(true); }} className="p-2.5 bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5 text-nexus-text hover:bg-nexus-accent hover:text-white rounded-xl transition-all backdrop-blur-sm border border-nexus-border/50 shadow-xl"><Edit2 size={14} /></button>
                    </div>
                  )}
        ...
        {activeTab === 'reports' && (
          <div className="p-10 grid grid-cols-1 md:grid-cols-2 gap-8 bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5 border-t border-nexus-border/30">
            <motion.div 
              initial={{ opacity: 0, x: -20 }}
              animate={{ opacity: 1, x: 0 }}
              className="bg-nexus-surface p-10 rounded-[2.5rem] border border-nexus-border/30 shadow-2xl flex flex-col items-center justify-center text-center group hover:border-nexus-success/30 transition-all relative overflow-hidden"
            >
            ...
            <motion.div 
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
              className="bg-nexus-surface p-10 rounded-[2.5rem] border border-nexus-border/30 shadow-2xl flex flex-col items-center justify-center text-center group hover:border-nexus-danger/30 transition-all relative overflow-hidden"
            >
            ...
            <motion.div 
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              className="md:col-span-2 bg-nexus-surface border border-nexus-border/30 p-12 rounded-[3.5rem] shadow-2xl flex flex-col sm:flex-row justify-between items-center gap-8 relative overflow-hidden group"
            >
      ...
      {selectedClientName && (
        <div className="fixed inset-0 bg-nexus-bg/80 backdrop-blur-xl z-[100] flex items-center justify-center p-6 overflow-hidden">
          <motion.div 
            ...
            className="bg-nexus-surface rounded-[3.5rem] p-12 max-w-3xl w-full shadow-[0_40px_100px_rgba(0,0,0,0.5)] border border-nexus-border/30 max-h-[90vh] overflow-y-auto custom-scrollbar relative"
          >
            ...
            <div className="relative z-10 flex justify-between items-start mb-12">
              ...
              <button 
                ...
                className="p-4 hover:bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5 rounded-2xl text-nexus-text-muted hover:text-white transition-all border border-transparent hover:border-nexus-border/50 shadow-2xl"
              >
              ...
              return (
                <div className="space-y-12 relative z-10">
                  ...
                  <div className="grid grid-cols-3 gap-6">
                    <div className="bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5 p-8 rounded-[2.5rem] border border-nexus-border/50 group hover:border-nexus-accent/30 transition-all">
                  ...
                  <div className="space-y-8">
                    <div className="flex items-center gap-4 mb-4">
                       ...
                       <div className="flex-1 h-px bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5" />
                    </div>
                    <div className="space-y-6">
                      {[...clientInvoices.map(i => ({...i, type: 'invoice'})), ...clientPayments.map(p => ({...p, type: 'payment'}))]
                        ...
                        .map((item: any, idx) => (
                          ...
                          >
                            ...
                            <div className="flex-1 bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5 border border-nexus-border/30 p-6 rounded-[2rem] shadow-2xl hover:border-white/20 transition-all">
                      ...
                      {clientInvoices.length === 0 && (
                        <div className="text-center py-20 bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5 rounded-[3rem] border-2 border-dashed border-nexus-border/50 opacity-30 italic">
      ...
      {isAddingCatalogItem && (
        <div className="fixed inset-0 bg-nexus-bg/80 backdrop-blur-xl z-[100] flex items-center justify-center p-6">
          <motion.div 
            ...
            animate={{ opacity: 1, scale: 1, y: 0 }}
            className="bg-nexus-surface rounded-[3.5rem] p-12 max-w-xl w-full shadow-[0_40px_100px_rgba(0,0,0,0.5)] border border-nexus-border/30 relative overflow-hidden"
          >
            ...
            <form onSubmit={handleUpdateCatalogItem} className="space-y-8 relative z-10">
              <div className="space-y-3">
                ...
                <input 
                  ...
                  className="w-full rounded-2x p-6 font-black placeholder:/30 transition-all italic text-lg tracking-tight hover:-strong nexus-input w-full text-sm" 
                  required 
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-3">
                  ...
                  <input 
                    ...
                    className="w-full rounded-2xl p-6 font-black transition-all italic text-lg hover:-strong nexus-input w-full text-sm" 
                    required 
                  />
                </div>
                {catalogType === 'product' && (
              ...
              <div className="grid grid-cols-2 gap-6 mt-12 pt-8 border-t border-nexus-border/30">
                <button type="button" onClick={() => setIsAddingCatalogItem(false)} className="py-5 bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5 text-nexus-text-muted font-black rounded-2xl text-[11px] uppercase tracking-[0.3em] hover:bg-slate-500/10 dark:bg-slate-500/10 dark:bg-white/10 transition-all border border-nexus-border/50 italic">Annuler</button>
      ...
      {isAddingPayment && selectedInvoice && (
        <div className="fixed inset-0 bg-nexus-bg/80 backdrop-blur-xl z-[100] flex items-center justify-center p-6">
          <motion.div 
            initial={{ opacity: 0, scale: 0.9, y: 20 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            className="bg-nexus-surface rounded-[3.5rem] p-12 max-w-2xl w-full shadow-[0_40px_100px_rgba(0,0,0,0.5)] border border-nexus-border/30 relative overflow-hidden"
          >
            ...
            <form onSubmit={handleCreatePayment} className="space-y-8 relative z-10">
              ...
              <div className="space-y-3">
                ...
                <input 
                  ...
                  className="w-full rounded-2xl p-6 font-black placeholder:/30 transition-all italic hover:-strong nexus-input w-full text-sm" 
                />
              </div>

              <div className="grid grid-cols-2 gap-6 mt-12 pt-8 border-t border-nexus-border/30">
                <button type="button" onClick={() => { setIsAddingPayment(false); setSelectedInvoice(null); }} className="py-5 bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5 text-nexus-text-muted font-black rounded-2xl text-[11px] uppercase tracking-[0.3em] hover:bg-slate-500/10 dark:bg-slate-500/10 dark:bg-white/10 transition-all border border-nexus-border/50 italic">Annuler</button>
            ...
            <form onSubmit={handleCreateSale} className="space-y-6 relative z-10">
              <div className="grid grid-cols-2 gap-6">
                 <div className="space-y-2">
                   ...
                   <select className="w-full rounded-2xl p-5 text-[13px] font-black transition-all italic hover:-strong nexus-input w-full text-sm" value={formData.type} onChange={e => setFormData({...formData, type: e.target.value})}>
                 ...
                 <div className="space-y-2">
                   ...
                   <input type="text" list="clients-list" placeholder="Anonyme..." className="w-full rounded-2xl p-5 text-[13px] font-black transition-all italic hover:-strong nexus-input w-full text-sm" value={formData.clientName || ''} onChange={e => setFormData({...formData, clientName: e.target.value})} />
                 </div>
              </div>

              <div className="space-y-2">
                 <label className="text-[11px] font-black text-nexus-text-muted uppercase tracking-[0.3em] ml-2 italic">Désignation Actif</label>
                 <input type="text" placeholder="Design Web, Ordinateur..." className="w-full rounded-2xl p-6 text-lg font-black transition-all italic tracking-tight hover:-strong nexus-input w-full text-sm" value={formData.itemName || ''} onChange={e => setFormData({...formData, itemName: e.target.value})} required/>
              </div>

              <div className="grid grid-cols-2 gap-6">
                 <div className="space-y-2">
                   <label className="text-[11px] font-black text-nexus-text-muted uppercase tracking-[0.3em] ml-2 italic">Fréquence/Qté</label>
                   <input type="number" step="0.5" className="w-full rounded-2xl p-6 text-lg font-black transition-all italic hover:-strong nexus-input w-full text-sm" value={formData.quantity || ''} onChange={e => setFormData({...formData, quantity: e.target.value})} required/>
                 </div>
                 <div className="space-y-2">
                   <label className="text-[11px] font-black text-nexus-text-muted uppercase tracking-[0.3em] ml-2 italic">Valence Unit. (F)</label>
                   <input type="number" step="0.01" className="w-full rounded-2xl p-6 text-lg font-black transition-all italic text-nexus-accent hover:-strong nexus-input w-full text-sm" value={formData.price || ''} onChange={e => setFormData({...formData, price: e.target.value})} required/>
                 </div>
              </div>

              <div className="pt-8 border-t border-nexus-border/30 flex justify-between items-center text-nexus-text">
              ...
              <div className="grid grid-cols-2 gap-6 mt-10">
                <button type="button" onClick={() => { setIsAdding(false); setEditingSale(null); setFormData({ type: 'product', quantity: 1, price: 0 }); }} className="py-5 bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5 text-nexus-text-muted font-black rounded-2xl text-[11px] uppercase tracking-[0.3em] hover:bg-slate-500/10 dark:bg-slate-500/10 dark:bg-white/10 border border-nexus-border/50 italic">Annuler</button>
                <button 
                  type="submit" 
                  disabled={submitting}
      ...
      {isAddingOrder && (
        <div className="fixed inset-0 bg-nexus-bg/80 backdrop-blur-xl z-[100] flex items-center justify-center p-6">
          <motion.div 
            initial={{ opacity: 0, scale: 0.9, y: 20 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            className="bg-nexus-surface rounded-[3.5rem] p-12 max-w-sm w-full shadow-[0_40px_100px_rgba(0,0,0,0.5)] border border-nexus-border/30 relative overflow-hidden"
          >
            ...
            <form onSubmit={handleCreateOrder} className="space-y-8 relative z-10">
              <div className="space-y-3">
                ...
                <input type="text" list="clients-list" placeholder="Nom ou ID..." className="w-full rounded-2xl p-6 text-lg font-black transition-all italic tracking-tight hover:-strong nexus-input w-full text-sm" value={newOrderName} onChange={e => setNewOrderName(e.target.value)} required/>
              </div>
              ...
              <div className="space-y-3">
                ...
                <input type="text" placeholder="Ex: Poste 7 ..." className="w-full rounded-2xl p-6 text-lg font-black transition-all italic hover:-strong nexus-input w-full text-sm" value={newOrderTable} onChange={e => setNewOrderTable(e.target.value)} />
              </div>

              <div className="grid grid-cols-2 gap-6 mt-12 pt-8 border-t border-nexus-border/30">
                <button type="button" onClick={() => setIsAddingOrder(false)} className="py-5 bg-slate-500/5 dark:bg-slate-500/5 dark:bg-white/5 text-nexus-text-muted font-black rounded-2xl text-[11px] uppercase tracking-[0.3em] hover:bg-slate-500/10 dark:bg-slate-500/10 dark:bg-white/10 border border-nexus-border/50 italic">Annuler</button>
                <button 
                  type="submit"
                  disabled={submitting}
                  className="py-5 bg-nexus-accent text-white font-black rounded-2xl text-[11px] uppercase tracking-[0.3em] hover:bg-blue-500 transition-all shadow-[0_20px_40px_rgba(91,140,255,0.3)] disabled:opacity-50 active:scale-95 italic"
