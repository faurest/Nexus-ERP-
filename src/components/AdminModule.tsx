...
export default function AdminModule() {
  ...
  const fetchGlobalData = async () => {
    try {
      ...
      // Fetch all memberships and companies from Supabase if available
      let supabaseCompanies = [];
      let supabaseMemberships = [];
      
      if (sb) {
        const { data: sCompanies } = await sb.from('companies').select('*');
        const { data: sMemberships } = await sb.from('company_members').select('*, users(*), roles(*)');
        supabaseCompanies = sCompanies || [];
        supabaseMemberships = sMemberships || [];
      }
      ...
      supabaseCompanies.forEach((sc: any) => {
      ...
      const mergedCompanies = Array.from(companyMap.values()).map(c => ({
        ...
        memberEmails: (supabaseMemberships || [])
          .filter((m: any) => m.company_id === c.id)
      ...
      supabaseMemberships.forEach((m: any) => {
        const u = m.users;
        if (!u) return;
