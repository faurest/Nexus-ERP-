...
export default function EcommerceModule({ user }: { user: any }) {
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const nexusCommission = Math.round(cartTotal * 0.04); // 4% commission for Nexus on sales
  ...
  return (
    <div className="space-y-6">
      ...
      {activeView === 'finances' && isAdmin && currentCompany && (
        <div className="max-w-5xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-slate-900 p-8 rounded-3xl border border-slate-800 shadow-xl shadow-slate-900/20 relative overflow-hidden group">
              ...
              <p className="text-4xl font-black text-white tracking-tight">
                 {orders.filter(o => o.status === 'DELIVERED').reduce((sum, o) => sum + (o.nexusCommission || Math.round((o.subtotal || o.total || 0) * 0.04)), 0).toLocaleString()} <span className="text-xl text-white/50">FCFA</span>
              </p>
            </div>
