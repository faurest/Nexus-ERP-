    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            {filteredClients.map((client) => (
              <TableRow key={client.id}>
                <div className="flex flex-col">
                  <span className="font-bold text-nexus-text">{client.name}</span>
                  <span className="text-[10px] text-nexus-text-muted font-bold uppercase tracking-tight">REF-{client.id.slice(0, 5)}</span>
                </div>
                <div className="space-y-0.5">
                  <div className="flex items-center gap-2 text-xs text-nexus-text-muted">
                    <Mail size={12} className="text-nexus-text-muted" /> <span>{client.email}</span>
                  </div>
                  {client.phone && (
                    <div className="flex items-center gap-2 text-xs text-nexus-text-muted">
                      <Phone size={12} className="text-nexus-text-muted" /> <span>{client.phone}</span>
                    </div>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  <Award size={14} className="text-amber-500" />
                  <span className="text-xs font-bold text-nexus-text-muted">{client.loyaltyPoints} pts</span>
                </div>
                <div className="flex items-center gap-2">
                  <TrendingUp size={14} className="text-green-500" />
                  <span className="text-sm font-bold text-nexus-text">{(client.salesTotal || 0).toLocaleString()} FCFA</span>
                </div>
                <div className="flex items-center gap-2">
                  <button 
        ...
        <div className="space-y-6">
          ...
          <div className="bg-nexus-accent text-white rounded-2xl p-6 shadow-xl shadow-slate-900/10">
            <h3 className="text-xs font-bold text-nexus-text-muted uppercase tracking-widest mb-4">Rétention Client</h3>
            ...
            <p className="mt-4 text-[10px] font-medium text-nexus-text-muted leading-relaxed uppercase tracking-wider">
          ...
          <div className="bg-nexus-surface rounded-2xl border border-white/10 p-8 max-w-lg w-full shadow-2xl border border-white/5">
            <div className="flex justify-between items-start mb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-bold text-nexus-text leading-none">{viewingClient.name}</h3>
                  <p className="text-sm font-medium text-nexus-text-muted/80 mt-1">Niveau: {viewingClient.loyaltyPoints} pts</p>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-6">
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-xl font-bold text-nexus-text">{editingClient ? 'Modifier Partenaire' : 'Enregistrement Client'}</h3>
                <p className="text-sm text-nexus-text-muted/80">{editingClient ? 'Mettre à jour les informations du client.' : 'Ajouter un nouveau partenaire commercial.'}</p>
              </div>
            </div>
